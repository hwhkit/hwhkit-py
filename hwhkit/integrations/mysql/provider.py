...
from hwhkit.integrations.mysql.config import MySQLConfig

if TYPE_CHECKING:
    from hwhkit.core.context import AppContext


class MySQLProvider(IntegrationProvider):
    """MySQL adapter (placeholder — not yet implemented)."""

    name: ClassVar[str] = "mysql"
    config_schema: ClassVar[type[BaseModel]] = MySQLConfig
    ...
    async def setup(self, ctx: AppContext) -> None:
        raise NotImplementedError(
            "MySQLProvider is a P2 placeholder; concrete adapter ships in a future release."
        )
    ...
    async def health_check(self) -> HealthStatus:
        return HealthStatus.fail("MySQLProvider is a P2 placeholder (not yet implemented)")
